...
export function useLazyData<T>(dataType: string, options: LazyDataOptions = {}): LazyDataResult<T> {
  const {
...
export function useLazySearch<T>(
  ...
  const { data, loading, hasMore, _error, totalCount, loadedCount, loadMore, refresh } =
    useLazyData<T>(dataType, options);
